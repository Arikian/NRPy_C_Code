...
static inline void normalize3(REAL a[3]) {
  const REAL n = norm3(a);
  if (n > 0.0) {
    a[0] /= n;
    a[1] /= n;
    a[2] /= n;
...
#pragma omp parallel for
    for (int i2 = NGHOSTS; i2 < Nxx_plus_2NGHOSTS2 - NGHOSTS; i2++) {
      ...
      for (int i1 = NGHOSTS; i1 < Nxx_plus_2NGHOSTS1 - NGHOSTS; i1++) {
        ...
        const REAL FDPart3tmp4 = ((hh) * (hh));
        const REAL FDPart3tmp5 = sin(xx1);
        const REAL FDPart3tmp3 = FDPart3tmp0 * (hDD00 + 1);
        const REAL FDPart3tmp7 = FDPart3tmp4 * ((FDPart3tmp5) * (FDPart3tmp5));
        const REAL FDPart3tmp2 = FDPart3tmp0 * hDD01 * hh;
        const REAL FDPart3tmp6 = FDPart3tmp0 * FDPart3tmp5 * hDD02 * hh;
