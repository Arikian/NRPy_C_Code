  ...
  for (int i2 = NGHOSTS; i2 < Nxx_plus_2NGHOSTS2 - NGHOSTS; i2++) {
    for (int i1 = NGHOSTS; i1 < Nxx_plus_2NGHOSTS1 - NGHOSTS; i1++) {
      ...
      for (int i0 = NGHOSTS; i0 < Nxx_plus_2NGHOSTS0 - NGHOSTS; i0++) {
        ...
        const REAL FDPart1tmp0 = -FDPart1_Rational_49_18 * hh;
        ...
        const REAL FDPart3tmp8 = pow(WW, -6);
        const REAL FDPart3tmp9 = ((hh) * (hh) * (hh) * (hh));
        const REAL FDPart3tmp10 = hDD00 + 1;
        const REAL FDPart3tmp20 = f1_of_xx1 * hDD02;
        const REAL FDPart3tmp21 = (1.0 / ((WW) * (WW) * (WW) * (WW)));
        const REAL FDPart3tmp28 = ((hh) * (hh) * (hh));
        const REAL FDPart3tmp39 = ((hh_dD2) * (hh_dD2));
        const REAL FDPart3tmp44 = ((hh_dD1) * (hh_dD1));
        const REAL FDPart3tmp57 = f1_of_xx1 * hh;
        const REAL FDPart3tmp62 = (1.0 / ((WW) * (WW) * (WW)));
        const REAL FDPart3tmp63 = 2 * hh;
        const REAL FDPart3tmp3 = FDPart3tmp1 * FDPart3tmp2;
        const REAL FDPart3tmp7 = FDPart3tmp0 * FDPart3tmp6;
        const REAL FDPart3tmp11 = FDPart3tmp9 * ((hDD12) * (hDD12));
        const REAL FDPart3tmp13 = FDPart3tmp2 * hDD11 + FDPart3tmp2;
        const REAL FDPart3tmp16 = FDPart3tmp2 * ((hDD01) * (hDD01));
        const REAL FDPart3tmp23 = FDPart3tmp2 * f1_of_xx1;
        const REAL FDPart3tmp51 = FDPart3tmp0 * FDPart3tmp2;
        const REAL FDPart3tmp59 = FDPart3tmp0 * hh;
        const REAL FDPart3tmp69 = 2 * FDPart3tmp62;
        const REAL FDPart3tmp5 = FDPart3tmp3 * hDD22 + FDPart3tmp3;
        const REAL FDPart3tmp12 = FDPart3tmp1 * FDPart3tmp10 * FDPart3tmp11 * FDPart3tmp8;
        const REAL FDPart3tmp14 = FDPart3tmp3 * ((hDD02) * (hDD02));
        const REAL FDPart3tmp24 = FDPart3tmp23 * hDD12;
        const REAL FDPart3tmp30 = -FDPart3tmp13 * FDPart3tmp20 * FDPart3tmp21 * hh + FDPart3tmp21 * FDPart3tmp28 * f1_of_xx1 * hDD01 * hDD12;
        const REAL FDPart3tmp31 = FDPart3tmp10 * FDPart3tmp13 * FDPart3tmp21 - FDPart3tmp16 * FDPart3tmp21;
        const REAL FDPart3tmp56 = FDPart3tmp0 * FDPart3tmp23;
        const REAL FDPart3tmp65 = FDPart3tmp20 * FDPart3tmp62 * FDPart3tmp63;
        const REAL FDPart3tmp70 = FDPart3tmp69 * partial_D_WW2;
        const REAL FDPart3tmp73 = FDPart3tmp62 * FDPart3tmp63 * hDD01;
        const REAL FDPart3tmp88 = FDPart3tmp69 * partial_D_WW1;
        const REAL FDPart3tmp95 = 2 * FDPart3tmp23 * f1_of_xx1__D1;
        const REAL FDPart3tmp100 = FDPart3tmp69 * partial_D_WW0;
        const REAL FDPart3tmp15 = FDPart3tmp13 * FDPart3tmp14 * FDPart3tmp8;
        const REAL FDPart3tmp17 = FDPart3tmp16 * FDPart3tmp5 * FDPart3tmp8;
        const REAL FDPart3tmp25 = -FDPart3tmp10 * FDPart3tmp21 * FDPart3tmp24 + FDPart3tmp2 * FDPart3tmp20 * FDPart3tmp21 * hDD01;
        const REAL FDPart3tmp41 = FDPart3tmp1 * FDPart3tmp21 * FDPart3tmp28 * hDD02 * hDD12 - FDPart3tmp21 * FDPart3tmp5 * hDD01 * hh;
        const REAL FDPart3tmp45 = FDPart3tmp10 * FDPart3tmp21 * FDPart3tmp5 - FDPart3tmp14 * FDPart3tmp21;
        const REAL FDPart3tmp47 = -FDPart3tmp1 * FDPart3tmp11 * FDPart3tmp21 + FDPart3tmp13 * FDPart3tmp21 * FDPart3tmp5;
        const REAL FDPart3tmp66 = FDPart3tmp0 * FDPart3tmp57 * partial_D_hDD202 - FDPart3tmp65 * partial_D_WW2;
        const REAL FDPart3tmp67 = 2 * FDPart3tmp30;
        const REAL FDPart3tmp71 = -FDPart3tmp24 * FDPart3tmp70 + FDPart3tmp56 * partial_D_hDD212;
        const REAL FDPart3tmp74 = FDPart3tmp59 * partial_D_hDD201 - FDPart3tmp73 * partial_D_WW2;
        const REAL FDPart3tmp77 = FDPart3tmp0 * partial_D_hDD200 - FDPart3tmp10 * FDPart3tmp70;
        const REAL FDPart3tmp79 = FDPart3tmp0 * FDPart3tmp3 * partial_D_hDD222 - FDPart3tmp5 * FDPart3tmp70;
        const REAL FDPart3tmp81 = -FDPart3tmp13 * FDPart3tmp70 + FDPart3tmp51 * partial_D_hDD211;
        const REAL FDPart3tmp84 = FDPart3tmp0 * (FDPart3tmp57 * partial_D_hDD102 + f1_of_xx1__D1 * hDD02 * hh) - FDPart3tmp65 * partial_D_WW1;
        const REAL FDPart3tmp86 = FDPart3tmp59 * partial_D_hDD101 - FDPart3tmp73 * partial_D_WW1;
        const REAL FDPart3tmp89 = FDPart3tmp0 * (FDPart3tmp2 * f1_of_xx1__D1 * hDD12 + FDPart3tmp23 * partial_D_hDD112) - FDPart3tmp24 * FDPart3tmp88;
        const REAL FDPart3tmp91 = FDPart3tmp0 * partial_D_hDD100 - FDPart3tmp10 * FDPart3tmp88;
        const REAL FDPart3tmp93 = -FDPart3tmp13 * FDPart3tmp88 + FDPart3tmp51 * partial_D_hDD111;
        const REAL FDPart3tmp96 = FDPart3tmp0 * (FDPart3tmp3 * partial_D_hDD122 + FDPart3tmp95 * hDD22 + FDPart3tmp95) - FDPart3tmp5 * FDPart3tmp88;
        const REAL FDPart3tmp98 = FDPart3tmp0 * (FDPart3tmp20 + FDPart3tmp57 * partial_D_hDD002) - FDPart3tmp65 * partial_D_WW0;
        const REAL FDPart3tmp101 = FDPart3tmp0 * (FDPart3tmp23 * partial_D_hDD012 + FDPart3tmp63 * f1_of_xx1 * hDD12) - FDPart3tmp100 * FDPart3tmp24;
        const REAL FDPart3tmp103 = FDPart3tmp0 * (hDD01 + hh * partial_D_hDD001) - FDPart3tmp73 * partial_D_WW0;
        const REAL FDPart3tmp105 = FDPart3tmp0 * partial_D_hDD000 - FDPart3tmp10 * FDPart3tmp100;
        const REAL FDPart3tmp107 =
            FDPart3tmp0 * (FDPart3tmp2 * partial_D_hDD011 + FDPart3tmp63 * hDD11 + FDPart3tmp63) - FDPart3tmp100 * FDPart3tmp13;
        const REAL FDPart3tmp110 = FDPart3tmp0 * (FDPart3tmp1 * FDPart3tmp63 * hDD22 + FDPart3tmp1 * FDPart3tmp63 + FDPart3tmp3 * partial_D_hDD022) -
                                   FDPart3tmp100 * FDPart3tmp5;
        const REAL FDPart3tmp18 = 2 * FDPart3tmp1 * FDPart3tmp8 * FDPart3tmp9 * hDD01 * hDD02 * hDD12 +
                                  FDPart3tmp10 * FDPart3tmp13 * FDPart3tmp5 * FDPart3tmp8 - FDPart3tmp12 - FDPart3tmp15 - FDPart3tmp17;
        const REAL FDPart3tmp35 = 2 * FDPart3tmp25;
        const REAL FDPart3tmp68 = FDPart3tmp66 * FDPart3tmp67;
        const REAL FDPart3tmp75 = 2 * FDPart3tmp41;
        const REAL FDPart3tmp78 = FDPart3tmp47 * FDPart3tmp77;
        const REAL FDPart3tmp80 = FDPart3tmp31 * FDPart3tmp79;
        const REAL FDPart3tmp82 = FDPart3tmp45 * FDPart3tmp81;
        const REAL FDPart3tmp85 = FDPart3tmp67 * FDPart3tmp84;
        const REAL FDPart3tmp92 = FDPart3tmp47 * FDPart3tmp91;
        const REAL FDPart3tmp94 = FDPart3tmp45 * FDPart3tmp93;
        const REAL FDPart3tmp97 = FDPart3tmp31 * FDPart3tmp96;
        const REAL FDPart3tmp99 = FDPart3tmp67 * FDPart3tmp98;
        const REAL FDPart3tmp106 = FDPart3tmp105 * FDPart3tmp47;
        const REAL FDPart3tmp108 = FDPart3tmp107 * FDPart3tmp45;
        const REAL FDPart3tmp111 = FDPart3tmp110 * FDPart3tmp31;
        const REAL FDPart3tmp19 = (1.0 / (FDPart3tmp18));
        const REAL FDPart3tmp72 = FDPart3tmp35 * FDPart3tmp71;
        const REAL FDPart3tmp76 = FDPart3tmp74 * FDPart3tmp75;
        const REAL FDPart3tmp87 = FDPart3tmp75 * FDPart3tmp86;
        const REAL FDPart3tmp90 = FDPart3tmp35 * FDPart3tmp89;
        const REAL FDPart3tmp102 = FDPart3tmp101 * FDPart3tmp35;
        const REAL FDPart3tmp104 = FDPart3tmp103 * FDPart3tmp75;
        const REAL FDPart3tmp112 = (1.0 / ((FDPart3tmp18) * (FDPart3tmp18)));
        const REAL FDPart3tmp32 = FDPart3tmp19 * FDPart3tmp31;
        const REAL FDPart3tmp46 = FDPart3tmp19 * FDPart3tmp45;
        const REAL FDPart3tmp113 = FDPart3tmp112 * ((FDPart3tmp41) * (FDPart3tmp41));
        const REAL FDPart3tmp114 = FDPart3tmp112 * ((FDPart3tmp45) * (FDPart3tmp45));
        const REAL FDPart3tmp115 = FDPart3tmp112 * ((FDPart3tmp25) * (FDPart3tmp25));
        const REAL FDPart3tmp116 = FDPart3tmp112 * FDPart3tmp25;
        const REAL FDPart3tmp118 = FDPart3tmp112 * FDPart3tmp45;
        const REAL FDPart3tmp120 = FDPart3tmp112 * FDPart3tmp41;
        const REAL FDPart3tmp123 = FDPart3tmp112 * FDPart3tmp47;
        const REAL FDPart3tmp125 = FDPart3tmp112 * FDPart3tmp30;
        const REAL FDPart3tmp133 = FDPart3tmp112 * ((FDPart3tmp30) * (FDPart3tmp30));
        const REAL FDPart3tmp134 = FDPart3tmp112 * ((FDPart3tmp31) * (FDPart3tmp31));
        const REAL FDPart3tmp136 = FDPart3tmp112 * FDPart3tmp31;
        const REAL FDPart3tmp146 = FDPart3tmp112 * ((FDPart3tmp47) * (FDPart3tmp47));
        const REAL FDPart3tmp27 = FDPart3tmp19 * FDPart3tmp25 * hh_dD1;
        const REAL FDPart3tmp38 = FDPart3tmp19 * FDPart3tmp30 * hh_dD2;
        const REAL FDPart3tmp43 = FDPart3tmp19 * FDPart3tmp41 * hh_dD1;
        const REAL FDPart3tmp52 = FDPart3tmp19 * FDPart3tmp25 * hh_dD2;
        const REAL FDPart3tmp117 = FDPart3tmp116 * FDPart3tmp75;
        const REAL FDPart3tmp121 = FDPart3tmp120 * FDPart3tmp30;
        const REAL FDPart3tmp122 = FDPart3tmp120 * FDPart3tmp25;
        const REAL FDPart3tmp126 = FDPart3tmp125 * FDPart3tmp45;
        const REAL FDPart3tmp128 = FDPart3tmp125 * FDPart3tmp25;
        const REAL FDPart3tmp130 = FDPart3tmp120 * FDPart3tmp31;
        const REAL FDPart3tmp135 = FDPart3tmp116 * FDPart3tmp67;
        const REAL FDPart3tmp141 = FDPart3tmp123 * FDPart3tmp25;
        const REAL FDPart3tmp147 = FDPart3tmp125 * FDPart3tmp75;
        const REAL FDPart3tmp152 = 2 * FDPart3tmp19 * FDPart3tmp30;
        const REAL FDPart3tmp153 = 2 * FDPart3tmp19 * FDPart3tmp41;
        const REAL FDPart3tmp34 = FDPart3tmp19 * FDPart3tmp30 - FDPart3tmp27 - FDPart3tmp32 * hh_dD2;
        const REAL FDPart3tmp49 = FDPart3tmp19 * FDPart3tmp35 * hh_dD1 * hh_dD2 + FDPart3tmp19 * FDPart3tmp47 + FDPart3tmp32 * FDPart3tmp39 -
                                  2 * FDPart3tmp38 - 2 * FDPart3tmp43 + FDPart3tmp44 * FDPart3tmp46;
        const REAL FDPart3tmp54 = FDPart3tmp19 * FDPart3tmp41 - FDPart3tmp46 * hh_dD1 - FDPart3tmp52;
        const REAL FDPart3tmp55 = FDPart3tmp19 * FDPart3tmp47 - FDPart3tmp38 - FDPart3tmp43;
        const REAL FDPart3tmp119 = -FDPart3tmp113 * FDPart3tmp91 - FDPart3tmp114 * FDPart3tmp93 - FDPart3tmp115 * FDPart3tmp96 -
                                   FDPart3tmp117 * FDPart3tmp84 - FDPart3tmp118 * FDPart3tmp87 - FDPart3tmp118 * FDPart3tmp90;
        const REAL FDPart3tmp129 =
        ...
        const REAL FDPart3tmp132 = -FDPart3tmp115 * FDPart3tmp71 - FDPart3tmp116 * FDPart3tmp80 - FDPart3tmp116 * FDPart3tmp82 -
        ...
        const REAL FDPart3tmp137 = -FDPart3tmp115 * FDPart3tmp81 - FDPart3tmp133 * FDPart3tmp77 - FDPart3tmp134 * FDPart3tmp79 -
                                   FDPart3tmp135 * FDPart3tmp74 - FDPart3tmp136 * FDPart3tmp68 - FDPart3tmp136 * FDPart3tmp72;
        const REAL FDPart3tmp139 = -FDPart3tmp115 * FDPart3tmp89 - FDPart3tmp116 * FDPart3tmp94 - FDPart3tmp116 * FDPart3tmp97 -
        ...
        const REAL FDPart3tmp143 = FDPart3tmp113 * FDPart3tmp86 + FDPart3tmp120 * FDPart3tmp92 + FDPart3tmp120 * FDPart3tmp94 +
        ...
        const REAL FDPart3tmp145 = FDPart3tmp120 * FDPart3tmp31 * FDPart3tmp71 + FDPart3tmp121 * FDPart3tmp74 + FDPart3tmp122 * FDPart3tmp81 +
        ...
        const REAL FDPart3tmp148 = FDPart3tmp101 * FDPart3tmp147 + FDPart3tmp104 * FDPart3tmp123 + FDPart3tmp105 * FDPart3tmp146 +
                                   FDPart3tmp107 * FDPart3tmp113 + FDPart3tmp110 * FDPart3tmp133 + FDPart3tmp123 * FDPart3tmp99;
        const REAL FDPart3tmp50 = (1.0 / (FDPart3tmp49));
        const REAL FDPart3tmp83 = (1.0 / sqrt(FDPart3tmp49));
        const REAL FDPart3tmp149 =
        ...
        const REAL FDPart3tmp156 = -FDPart3tmp113 * FDPart3tmp81 - FDPart3tmp123 * FDPart3tmp68 - FDPart3tmp123 * FDPart3tmp76 +
        ...
        const REAL FDPart3tmp158 = -FDPart3tmp113 * FDPart3tmp93 + FDPart3tmp119 * FDPart3tmp44 - FDPart3tmp123 * FDPart3tmp85 -
        ...
        const REAL FDPart3tmp61 = 2 * FDPart3tmp50 * FDPart3tmp55;
        const REAL FDPart3tmp150 = (1.0 / 2.0) * FDPart3tmp83;
